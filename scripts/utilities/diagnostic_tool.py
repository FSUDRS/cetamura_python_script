    ...
    try:
        ...
        for year_dir in base_path.iterdir():
            ...
            for subfolder in year_dir.iterdir():
                ...
                print(
                    f"      📋 Manifest files: {len(manifest_files)} {[f.name for f in manifest_files]}"
                )
                ...
                if len(manifest_files) == 0:
                    print(
                        "      🚨 NO MANIFEST.ini FOUND - This should cause MANIFEST_ERROR"
                    )
                elif len(manifest_files) > 1:
                    print(
                        "      🚨 MULTIPLE MANIFEST FILES - This should cause MANIFEST_ERROR"
                    )
                else:
