...
"""
    ...
    for patent_id in patent_ids:
        (patent_batch_dir / f"{patent_id}.xml").write_text(
            create_patent_xml(patent_id), encoding="utf-8"
        )
...
def test_batch_process_multi_file_dry_run(setup_multi_file_directory):
    ...
    success_count, error_count, csv_path = batch_process_with_safety_nets(
        folder_path=str(tmp_path), dry_run=True, staging=False
    )
...
def test_batch_process_multi_file_staging(setup_multi_file_directory):
    ...
    success_count, error_count, csv_path = batch_process_with_safety_nets(
        folder_path=str(tmp_path), dry_run=False, staging=True
    )
...
def test_batch_process_multi_file_production_is_non_mutating(
    setup_multi_file_directory,
):
    """Production mode should also leave source files unchanged."""
    tmp_path, photo_set_dir, test_files = setup_multi_file_directory

    success_count, error_count, csv_path = batch_process_with_safety_nets(
        folder_path=str(tmp_path), dry_run=False, staging=False
    )
...
def test_file_matching_by_stem(setup_multi_file_directory):
    ...
    for xml_file in photo_set.xml_files:
        ...
        assert (
            len(matching_jpgs) == 1
        ), f"Expected exactly 1 matching JPG for {xml_file.name}"
...
def test_patent_batch_processing_skips_photo_recovery_scan(
    setup_patent_batch_directory, monkeypatch
):
