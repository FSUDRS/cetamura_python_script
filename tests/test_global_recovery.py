...
def test_global_recovery_strategy(setup_sandbox):
    ...
    assert (
        expected_zip.exists()
    ), "ZIP file should be created despite cross-folder separation"
    assert (
        root_dir / "folder_b" / "TEST_IID_001.jpg"
    ).exists(), "Recovered source image should remain in place"
    assert (
        root_dir / "folder_a" / "TEST_IID_001.xml"
    ).exists(), "Source XML should remain in place"
...
def test_global_recovery_reporting(setup_sandbox):
    ...
    assert (
        "folder_b" in report_content
    ), "Report should mention the source folder of the recovered image"
