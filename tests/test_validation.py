...
class TestValidateBatchOutput:
    ...
    def test_matching_counts_passes(self, tmp_path):
        ...
        result = validate_batch_output(
            photo_sets, tmp_path, success_count=3, dry_run=False
        )
...
class TestReconciliationReport:
    ...
    def test_discrepancy_detection_missing_zips(self, tmp_path):
        ...
        assert len(report.discrepancies) > 0
        assert any(
            "CSV SUCCESS rows" in d and "Actual ZIP count" in d
    ...
    def test_discrepancy_detection_invalid_zips(self, tmp_path):
        ...
        assert any(
            "Actual ZIP count" in d and "Valid ZIP count" in d
            for d in report.discrepancies
        )
    ...
    def test_orphaned_files_detection(self, tmp_path):
        ...
        report = generate_reconciliation_report(
            photo_sets, csv_path, tmp_path, work_root=tmp_path / ".work"
        )
