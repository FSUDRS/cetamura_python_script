...
class TestExpandedSupport(unittest.TestCase):
    ...
    def create_dummy_xml(self, name, iid):
        ...
        ident = ET.SubElement(
            root, "{http://www.loc.gov/mods/v3}identifier", type="IID"
        )
        ident.text = iid
    ...
    def test_png_processing(self):
        ...
        batch_process_with_safety_nets(
            str(self.input_dir), dry_run=False, staging=False
        )
    ...
    def test_pdf_processing(self):
        ...
        success, errors, _ = batch_process_with_safety_nets(
            str(self.input_dir), dry_run=False, staging=False
        )
        ...
        if success > 0:
            ...
            self.assertTrue(
                zip_file.exists(), "PDF ZIP not created despite success report"
            )
        else:
